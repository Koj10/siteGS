  ...
  let app = new Vue({
    ...
    methods: {
      ...
      async ensureSpinPermission() {
        // Проверяем доступность вращения через API
        ...
        try {
          const apiBase = (window.GS_API_BASE || "http://193.176.78.125:6000").replace(/\/+$/, "");
          const { data } = await axios.get(`${apiBase}/roulette/spin`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
          if (data && typeof data.spin !== 'undefined' && data.spin === false) {
      ...
      prize_draw(num) {
        ...
        console.log('Количество призов', vm.numbers)
